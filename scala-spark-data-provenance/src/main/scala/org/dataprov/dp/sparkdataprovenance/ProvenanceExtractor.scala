...
object ProvenanceExtractor {
  ...
  /** Extract the minimal datasets from the source DataFrames that are necessary
    ...
    *
    * @param finalDF
    *   The final tagged DataFrame from the pipeline
    * @param sourceDFs
    *   The list of original source DataFrames (must have their ID/UUID column)
    * @param provenanceColName
    *   The name of the provenance column (e.g., "_provenance_tag")
    * @return
    *   A list of DataFrames containing a set of rows necessary to reproduce the
    *   final DataFrame
    */
  def extractMinimalDatasets(
    ...
    sourceDFs.map { sourceDF =>
      ...
      val idColumnName =
        if (sourceDF.columns.contains(provenanceColName)) provenanceColName
        else "id"
