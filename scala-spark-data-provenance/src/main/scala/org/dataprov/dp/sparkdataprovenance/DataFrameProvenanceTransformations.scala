...
object DataFrameProvenanceTransformations {
    ...
    def addProvenanceColumn(spark: SparkSession, dfName: String): String = {
        ...
        spark.table(dfName).withColumn(colName, provenanceColumn).createOrReplaceTempView(dfName)
        dfName
    }
    ...
    implicit class DataFrameWithProvenance(df: DataFrame) {
        def addProvenanceColumn: DataFrame = DataFrameProvenanceTransformations.addProvenanceColumn(df)
        def removeProvenanceColumn: DataFrame = DataFrameProvenanceTransformations.removeProvenanceColumn(df)
    }
}
